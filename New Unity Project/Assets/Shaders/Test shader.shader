Shader "Custom/Combined Shader" {
	...
	SubShader 
	{
		...
		struct appdata
		{
			...
			float2 uv : TEXCOORD0; // dissolve
		};

		struct v2f
		{
			float2 uv : TEXCOORD0; //dissolve
			float3 worldPos : TEXCOORD1; //dissolve
			float4 vertex : SV_POSITION; //dissolve
			float4 pos : POSITION;
			float4 color : COLOR;
			float3 normal: NORMAL;
		};
		...
		v2f vert(appdata v)
		{
			...
			o.vertex = UnityObjectToClipPos(v.vertex); //Dissolve
			o.uv = TRANSFORM_TEX(v.uv, _MainTex);	//Dissolve
			o.worldPos = mul(unity_ObjectToWorld, v.vertex).xyz; // Dissolve
		...
		Pass // render the outline 
		{
			...
			half4 frag(v2f i) : COLOR
			{
				...
				if(_DissolveSize > 0)
				{
					float transition = tex2D(_DissolveTex,i.uv);
					clip(transition);
				}
